...
: 2020?       + 2020 = ;
: find-match? begin 2dup @ 2020? if @ nip exit then dup @ 0<> while cell+ repeat 2drop false ;
: find-pair   begin dup @ over find-match? ?dup if swap @ exit else cell+ then again ;
